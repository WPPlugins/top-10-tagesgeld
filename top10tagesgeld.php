...
function top10_tagesgeld_init() {
	...
	function top10_tagesgeld($args) {
		...
		// *.css - Datei einbinden
		// echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"top_10_tagesgeld.css\">";
		...
		if(@fopen("http://www.toptentagesgeld.de/top10.txt","r")){
			...
			foreach ($csv as $row) :  
				// echo "<tr><td width=75%><a href=$row[2]>$row[0]</a></td><td align=right width=25%>$row[1] %</td></tr>";
	...
	function top10_tagesgeld_control() {
   	...
   	} // End of top10_tagesgeld_control()
